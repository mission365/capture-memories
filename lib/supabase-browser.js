...
export async function updateHeroSlide(id, payload, accessToken) {
  const selector = typeof id === 'object' && id !== null ? id : { id };
  return localApiRequest('/api/admin/hero-slides', {
    method: 'PATCH',
    headers: createLocalApiHeaders({ accessToken }),
    body: JSON.stringify({ id: selector.id || '', selector, payload }),
  });
}
...
export async function deleteHeroSlide(id, accessToken) {
  ...
  return localApiRequest('/api/admin/hero-slides', {
    method: 'DELETE',
    headers: createLocalApiHeaders({ accessToken }),
    body: JSON.stringify({ id: selector.id || '', selector }),
  });
}
...
function assertUploadSize(file) {
  ...
  throw new Error(
    `${fileLabel} is ${formatFileSize(file.size)}. Please upload an image smaller than ${formatFileSize(MAX_UPLOAD_SIZE_BYTES)}.`
  );
}
