...
export function normalizeFeaturedAlbum(album = {}, index = 0) {
  ...
  return {
    ...
    pageCaption: readText(album?.pageCaption || album?.caption),
    story: readText(album?.story),
    credit: readText(album?.credit),
    image: normalizeSupabaseStorageUrl(readText(album?.image || album?.imageUrl)),
  };
}
...
export function normalizeAlbumStoryGalleryItem(item = {}) {
  return {
    image: normalizeSupabaseStorageUrl(readText(item?.image || item?.imageUrl)),
